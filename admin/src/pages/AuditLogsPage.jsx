...
export function AuditLogsPage() {
  ...
  const { data, isPending, isError, error } = useQuery({
    ...
    queryFn: () => {
      const q = new URLSearchParams({ skip: String(pageIndex * pageSize), take: String(pageSize) })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-mono text-2xl font-bold uppercase tracking-tight text-catskill">Логи</h1>
      ...
      {isError ? (
        <div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-6 text-sm text-red-200">
          <p className="font-medium">{error instanceof Error ? error.message : 'Ошибка загрузки'}</p>
          {auditErrorHint ? <p className="mt-3 text-xs leading-relaxed text-red-300/90">{auditErrorHint}</p> : null}
        </div>
      ) : null}
        ...
        <>
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody>
                {isPending
                  ...
                  : items.map((row) => (
                      <tr key={row.id} className="border-b border-plantation/40 align-top hover:bg-white/[0.02]">
                        <td className="whitespace-nowrap p-3 font-mono text-xs text-gull">
                          {new Date(row.createdAt).toLocaleString()}
                        </td>
                        <td className="p-3">
                          <div className="font-mono text-xs text-turquoise">@{row.actor?.handle}</div>
                          <div className="text-[10px] text-slate-arena">{roleLabel(row.actor?.role)}</div>
                        </td>
          ...
          <div className="flex items-center justify-between gap-4">
            <p className="text-xs text-gull">
              Стр. {pageIndex + 1} / {pageCount} · записей: {total}
            </p>
