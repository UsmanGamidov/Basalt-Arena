...
- `npm run format` / `npm run format:check` — единый Prettier-стандарт в монорепо.
- `npm run lint` — ESLint для `bff`, `client`, `admin` из общего root-конфига.
- `npm run typecheck` — строгая TS-проверка BFF (`tsc --noEmit`).
- `npm run verify` — полный quality gate: формат + lint + typecheck + unit tests BFF.

Pre-commit:

- Подключены `husky` + `lint-staged`.
- Хук `.husky/pre-commit` запускает `npm run precommit:check`, который выполняет:
  1. формат/линт только изменённых файлов;
  2. typecheck BFF;
  3. unit-тесты BFF.
